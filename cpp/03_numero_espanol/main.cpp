...
int main() {
    ...
    std::cout << "Servidor C++ local en español ejecutándose en http://localhost:3063" << std::endl;
    ...
    while (true) {
        ...
        if (n == -999999) {
            responder(cliente, "Debes enviar un número. Ejemplo: http://localhost:3063/?n=10");
        } else {
            std::string espanol = numeroAEspanol(n); responder(cliente, "Número: " + std::to_string(n) + "\nResultado local: " + espanol);
        }
        close(cliente);
    }
}
