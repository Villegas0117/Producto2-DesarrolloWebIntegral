...
int main() {
    ...
    std::cout << "Servidor C++ SOAP en inglés ejecutándose en http://localhost:3061" << std::endl;
    ...
    while (true) {
        ...
        if (n == -999999) {
            responder(cliente, "Debes enviar un número. Ejemplo: http://localhost:3061/?n=10");
        } else {
            std::string ingles = consultarSOAP(n); responder(cliente, "SOAP inglés: " + ingles);
        }
        close(cliente);
    }
}
