...
print "Servidor Perl SOAP traducido ejecutándose en http://localhost:3022\\n";
...
while (my $client = $server->accept()) {
    ...
    my $body;
    if ($n eq "") {
        $body = "Debes enviar un número. Ejemplo: http://localhost:3022/?n=10";
    } else {
        my $ingles = consultar_soap($n); my $espanol = numero_a_espanol($n); $body = "SOAP inglés: $ingles\nTraducción al español: $espanol";
    }
