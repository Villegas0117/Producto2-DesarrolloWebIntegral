        ...
        server.createContext("/", exchange -> {
            try {
                ...
                if (query == null || !query.startsWith("n=")) {
                    responder(exchange, "Debes enviar un número. Ejemplo: http://localhost:3053/?n=10");
                    return;
                }
                int n = Integer.parseInt(query.replace("n=", ""));
                String espanol = SpanishNumber.numeroAEspanol(n); responder(exchange, "Número: " + n + "\nResultado local: " + espanol);
            } catch (Exception e) {
                responder(exchange, "Error: " + e.getMessage());
            }
        });
        System.out.println("Servidor Java local en español ejecutándose en http://localhost:3053");
        server.start();
    }
